<template>
	<v-container fluid class="main-content">
		<v-row no-gutters class="mt-4" v-if="itemsInCart.length">
					...
					<v-col cols="12">
						...
						>
							...
							<template v-slot:item.quantity="{ item }">
								<v-text-field class="quantity_input text-center" readonly disabled color="#ea2d0f" dense v-model="item.quantity">
									<v-icon slot="append" class="pt-1 pl-1" color="red" small @click="increaseQuantity(item)">fas fa-plus</v-icon>
		...
		<v-row no-gutters v-if="itemsInCart.length" class="mt-4">
					...
					<v-col cols="12" class="d-flex justify-end mt-8">
						<v-slide-y-transition leave-absolute hide-on-leave group>	
							<v-btn
								...
								редактировать заказ <v-icon class="ml-2">fas fa-edit</v-icon> 
							</v-btn>
							<v-btn
								key="save-data"
								class="confirm-btn px-4"
								...
								@click.stop="comfirmPersonal"
								v-if="!GETTER_DISABLED_MARK && GETTER_CHECKED_DELIVERY">
								подтвердить доставку <v-icon class="ml-2">fas fa-check</v-icon>
							</v-btn>
		...
		<v-row no-gutters v-if="itemsInCart.length">
		...
		<v-row no-gutters v-if="!itemsInCart.length" class="main-content__block empty-cart mx-12 my-8">
				<v-col cols="12">
					<v-img
						:aspect-ratio="1/2"
...
export default {
	...
	computed: {
		...
		cartItemsInTable() {
			...
			this.itemsInCart.map((item) => {
				let itemInCart = {
					id: item.id,
					image: this.checkImgInCart(item),
					title: item.title,
					link: item.link,
	...
	methods: {
		...
		...mapActions('cart', ['SAVE_NEW_ORDER']),
		...
		calcOrderSumm() {
			this.check.orderSum = this.itemsInCart.reduce((sum, current) => {				
			...
			if(this.GETTER_CHECKED_DELIVERY) {
				...
				if(currentDelivery[0].costProcent) { // delivery like procent					
					this.check.deliverySum = 
						((parseFloat(this.check.orderSum) * currentDelivery[0].cost/100).toFixed(2) > 5) ? 
						(parseFloat(this.check.orderSum) * currentDelivery[0].cost/100).toFixed(2) : 5
				}
				else if(currentDelivery[0].freeCostfrom) {
					this.check.deliverySum = (parseFloat(this.check.orderSum) < currentDelivery[0].freeCostfrom) ? 
						(currentDelivery[0].cost).toFixed(2) : 0
				}
				else {
		...
		async checkOrder() {
			...
			if(this.$refs.cartForm.$v.$invalid) {
				...
				// 		orderTime: new Date(),
				// 		status: 1,
				// 		orderedItems: this.itemsInCart.map(item => {return item.id}),
				// 		orderFields: this.GETTER_ORDER_FIELDS
				// 	}
				// }

				let currentOrder = {
					orderNumber: currentDate.split(' ').shift() + '/' + Math.floor(Math.random() * (9999 - 1000 + 1)),
				...
				try {
					...
					await this.SAVE_NEW_ORDER(currentOrder)
					
					this.CLEAR_CART()

					this.$router.push({ name: 'checkout', params: { orderNumber: currentOrder.orderNumber } })
				}
				catch (err) {
					if (process.browser) {
						setTimeout(() =>
