	...
	<div class="gallery-wrapper">
		<div v-swiper:mainSwiper="mainSwiperOption" class="gallery-main" ref="mainSwiper">
			<div class="swiper-wrapper">
				<div 
					...
					:key="i" 
					@click="(banners[0].src.includes('noProductImage') && banners.length==1) ? null : zoomSlide()" 
			...
			<div class="swiper-button-prev" v-if="this.banners.length > 1"></div>
			<div class="swiper-button-next" v-if="this.banners.length > 1"></div>
		</div>
		...
		>
			<v-card raised class="gallery-modal d-flex flex-column pa-4 pa-sm-12">
				...
				<v-icon class="close-modal justify-end pr-4 pt-4" @click="closeSlide()">fas fa-times</v-icon>
	...
	data: function() {
		return {
			...
			thumbSwiperOption: {
				...
				observeSlideChildren: true,
				spaceBetween: 10,
				slidesPerView: this.banners.length,
				centeredSlides: true,
				touchRatio: 0.2,
				loopedSlides: this.banners.length,
				loop:  true,
			...
			thumbSwiperInZoomOption: {
				...
				breakpoints: {
					640: {
						slidesPerView: this.banners.length - 1,
					}
				},
			},
		}
